...
def view_chart_of_accounts_none(request):
    view=Chart_of_Account.objects.filter(status="None",user=request.user.id)
    return render(request,'chartofaccount_home.html',{"view":view})

def delet_chart(request,id):
    d=Chart_of_Account.objects.get(id=id)
    d.delete()
...
'''login_required(login_url='login')
def view_manual_journal_all(request):
    view=JournalEntry.objects.all()
    return render(request,'manual_journal.html',{"view":view})'''

'''login_required(login_url='login')
def view_journal_all(request):
    view=JournalEntry.objects.all()
    return render(request,'journal_list.html',{"view":view})   
    
login_required(login_url='login')
def view_journal_save(request):
    view=JournalEntry.objects.filter(journal__status="save")
    return render(request,'journal_list.html',{"view":view}) 

login_required(login_url='login')
def view_journal_draft(request):
    view=JournalEntry.objects.filter(journal__status="draft")
    return render(request,'journal_list.html',{"view":view})'''
...
'''def manual_journal_home(request):
    ...
    if filter_type == 'draft':
        journals = journals.filter(journal__status='draft')
    elif filter_type == 'save':
        journals = journals.filter(journal__status='save')
...
'''def create_edit_manual_journal(request,pk):
    ind=Journal.objects.get(id=pk)
    return render(request,'journal_list.html',{"ind":ind})'''    
...
def view_journal(request,jid):
    #journals=Journal.objects.all()
    jo=Journal.objects.get(id=jid)
    views=Journal.objects.filter(user=request.user.id)
    company=company_details.objects.get(user_id=request.user.id)
    context={"jo":jo,'views':views,'company':company}
    return render(request, 'journal_list.html', context)'''
...
def update_journal_status(request,id):
    ...
    jo.status = "save"
    jo.save()
    return redirect('journal_list', id)

'''def update_journal_status(request, journal_id):
    if request.method == 'POST':
        try:
            selected_journal = JournalEntry.objects.get(id=journal_id)
            selected_journal.journal.status = 'save'
